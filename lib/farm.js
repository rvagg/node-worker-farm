...
Farm.prototype.startChild = function () {
  var childData = fork(this.path, this.receive.bind(this), this.onExit.bind(this, this.childId));
...
// if the child is still alive after the timeout, use SIGKILL to make absolutely sure it dies
Farm.prototype.stopChild = function (childId) {
  var child = this.children[childId];
  if (child) {
    child.send('die')
    setTimeout( function killChild(){
      if( this.children[childId] ) process.kill( child.pid, 'SIGKILL' )
    }.bind(this), 150 );
...
Farm.prototype.childTimeout = function (childId) {
  ...
  for (i in child.calls) {
    this.receive({
        idx   : i
      , child : childId
      , args  : [ new Error('worker call timed out!') ]
    })
  }
  this.stopChild(childId,true)
}
